...
def pregrasp_l5_submission_order(
    candidates: Sequence[Mapping[str, Any]],
    *,
    prior_attempts: Sequence[Mapping[str, Any]] = (),
) -> list[JsonDict]:
    """Quality-sort L5 work while reserving the second slot for diversity.

    The joint funnel has one global two-plan budget. A plain physical-quality
    sort can spend both plans on near-duplicate goals from the first grasp.
    Greedy novelty first prefers a new grasp and placement cluster, then a new
    grasp, then a new cluster. Quality remains deterministic within each tier.
    """
