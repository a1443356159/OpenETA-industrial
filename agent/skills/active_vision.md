...
no mask, the host may instead bind the retained calibrated image point for one
semantic-search attempt. The agent decides that another view is useful; the
...
host supplies `semantic_target` and `target_hint`. Use
...
a host-bound visual point, or merely to obtain a prettier image. Preserve the
