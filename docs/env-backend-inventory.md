...
| `gazebo` | Gazebo Sim (Harmonic) / ROS 2 Jazzy + MoveIt 2 | `sim/env_registry.py:_register_gazebo_envs` registers `openeta/gazebo_live_rgbd-v0`, `openeta/gazebo_rm75_robotiq2f85-v0`, and the stable-but-disabled `openeta/gazebo_rm75_robotiq2f85_pickplace-v0`. M1/M2 construct `extensions/gazebo/direct_env.py:GazeboDirectEnv` (wrapped in `UnifiedEnv`); M3 rejects construction with `DETACHABLE_JOINT_UNIMPLEMENTED_OR_UNAPPROVED`. | OpenETA-native, not RLinf-derived. M1/M2 are profile-driven and `GazeboRuntime` owns their launch, RGB-D cameras and controller. M3 scene metadata remains only for offline Oracle contracts; it has no manipulation fallback. See `docs/gazebo-adapter-design.md`. |
