...
两次记录在下文的物理执行使用代码提交
`7cbe9ef9e751322771a2d6aeaa80ac13722277b1`。当前运行代码另包含 `46429fa` 的成功回执
上下文去重；该变更已由本地 Planner/会话测试覆盖，但尚未被重新计为一次新的物理 PASS。
`0f678b4` 与本文档提交只改变交付材料，不改变运行代码。
...
这是一项通用的上下文投影优化，不改变工具、候选、物理控制或验收条件。对应的会话和
Planner 投影单元测试已通过；由于不需要启动新场景来验证文本去重，物理稳定性依据仍以上述
两次真实 PASS 为准。
