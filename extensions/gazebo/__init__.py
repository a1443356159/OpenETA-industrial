...
try:  # gymnasium is an optional runtime dependency for the worker process
    from .worker import GazeboM2WorkerEnv, GazeboRobotiq2F85WorkerEnv, GazeboWorkerEnv, live_session_config_from_env
except ModuleNotFoundError as exc:  # keep asset/contracts importable offline
    if exc.name != "gymnasium":
        raise
    GazeboM2WorkerEnv = GazeboRobotiq2F85WorkerEnv = GazeboWorkerEnv = None  # type: ignore[assignment]
